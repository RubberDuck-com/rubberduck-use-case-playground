# UC-02: Codebase Audit (Security Audit)

## What this does

Find security-sensitive paths with source-to-sink evidence (not pattern-only noise).

## What you gain

A defensible security report: each finding has entry → data-flow → sink, severity, and file:line citations.
...
Optional live API: `python scripts/run-lab.py --uc 02 --server` then open http://127.0.0.1:8080/docs
...
- **Edit only:** `target=` is pre-filled. Optional: change `focus=` for a narrower scope.
- Copy the entire **Playground prompt** block below

## What a good answer looks like

`SECURITY_AUDIT.md` with findings table; ends with `RubberDuck initialization: passed` or a clear failure.

Must include: repo name, branch/ref, **pinned commit SHA**, and **file:line** evidence.

**Focus files in this repo:** `demoapp/config.py` — `eval_config_file`, `from_pickle`
